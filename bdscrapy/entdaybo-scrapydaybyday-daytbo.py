...
def main():
    ...
    #page_results = [[0 for col in range(2)] for row in range(300)]
    workbook = xlrd.open_workbook('C:/Users/dell/PycharmProjects/project-scrapy/test2.xls')  # 打开excel文件
    #sheets = workbook.sheet_names()  # 获取工作簿中的所有表格
    # worksheet = workbook.sheet_by_name(sheets[0])  # 获取工作簿中所有表格中的的第一个表格
    worksheet = workbook.sheet_by_index(0)  # 获取工作簿中所有表格中的的第一个表格
    rows_old = worksheet.nrows
    ...
    while dateymd  != '2018-12-31':
        ...
        dateymdexcel = datetime.strptime(dateymd , "%Y-%m-%d")
        print("日期是%s" % dateymdexcel)

        tboexcel = driver.find_element_by_xpath('//*[@id="Info_SummaryNum"]').text
        print("日总票房是%s" % tboexcel)

        new_worksheet.write(row + rows_old, 0, dateymdexcel)
        new_worksheet.write(row + rows_old, 1, tboexcel)
        row += 1
        new_workbook.save('C:/Users/dell/PycharmProjects/project-scrapy/test2.xls')
    #driver.close()
