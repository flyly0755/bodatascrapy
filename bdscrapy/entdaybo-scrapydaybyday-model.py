...
def dayscrapy(driver):
    ...
    page_results = [[0 for col in range(excelcolunm)] for row in range(elementcounts)]
    ...
    daytbo= driver.find_element_by_xpath('//*[@id="Info_SummaryNum"]').text #日总票房
    ...
    for i in range(1, elementcounts+1):
        ...
        #print("i-1得值为",i-1)
        try:
            page_results[i-1][2]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div/ul[%d]' % i).get_attribute('data-ent') #电影编号
            #print("电影编号是：%s" %page_results[i-1][1])
            page_results[i-1][3]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div/ul[%d]/li[1]/div/p[1]' % i).text #电影片名
            ...
            page_results[i-1][5]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[1]/div/p[2]/span[1]' % i).text #上映天数
            #print("上映天数是：%s" % page_results[i-1][4])
            page_results[i-1][6]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[1]/div/p[2]' % i).text #上映总票房(亿万)
            #print("上映总票房(亿万)是：%s" % page_results[i-1][5])
            page_results[i-1][7]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[2]/div' % i).get_attribute('textContent') #当日票房
            #print("当日票房是：%s" % page_results[i-1][6])
            page_results[i-1][8]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[3]' % i).get_attribute('textContent') #当日场次
            #print("当日场次是：%s" % page_results[i-1][7])
            page_results[i-1][9]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[4]' %i).get_attribute('textContent') #当日排座
            #print("当日排座是：%s" % page_results[i-1][8])
            page_results[i-1][10]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[5]' %i).get_attribute('textContent') #当日人次
            #processedcontent1 = re.findall('"data":.*?]}]', rowcontent, re.S)
            page_results[i-1][11]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[6]' %i).get_attribute('textContent') #累计票房（万）
            page_results[i-1][12]=driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[7]' %i).get_attribute('textContent') #当日票房占比
            page_results[i-1][13] = driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[8]' % i).get_attribute('textContent')  # 当日场次占比
            page_results[i-1][14] = driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[9]' % i).get_attribute('textContent')  # 上座率
            page_results[i-1][15] = driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[10]' % i).get_attribute('textContent')  # 当日排座占比
            page_results[i-1][16] = driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[11]' % i).get_attribute('textContent')  # 平均票价
            page_results[i-1][17] = driver.find_element_by_xpath('//*[@id="TableWrap"]/div[1]/div[2]/ul[%d]/li[12]' % i).get_attribute('textContent')  # 场均人次


            for index in range(18):  # 依次写入每一行
